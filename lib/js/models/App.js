...
(function($, _, Converter, CategoryMenu, ConversionWindow, Calculator, Conversions, UnitPicker){
    ...
    _.extend(App.prototype, {
        ...
        updateConversions: function () {
            ...
            var rational = $$.toWholeAndRational($$.roundDecimal(converted, 4), 4);

            this.conversion.setToValue($$.roundDecimal(converted, 4));

            console.log('Convert ' + this.conversion.getFromValue(true) + this.conversion.getFromUnit() + ' = ' + converted + this.conversion.getToUnit() + ' (' + rational.whole + ' ' + rational.num + '/' + rational.den + ')');
        },
        ...
        _handleChooseToUnit: function () {
            ...
            view.$el.one('new:unit', function (e, unit) {
                self.conversion.setToUnit(unit, self.converter.getUnits()[unit]);
                self.updateConversions();
                view.destroy();
            });
        ...
        _handleAdd: function () {
            ...
            self.calc.$el.one('new:operation clear', function () {
